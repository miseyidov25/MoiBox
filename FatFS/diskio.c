...
// NOTE 2. Implement the macros `CS_H()` and `CS_L()` to control the CS pin
//         using the GPIO module. The CS_INIT() macro can be omitted.
...
// NOTE 3. in the xmit_mmc() function, use the appropriate LPSPI transmit
...
// NOTE 4. in the rcvr_mmc() function, use the appropriate LPSPI receive
...
)
{
    ...
    // NOTE 5. Initialize the LPSPI and set the LPSPI dummy byte to 0xFF
    lpspi_master_init();
